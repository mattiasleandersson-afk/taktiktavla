...
/* === v96-formation-compact-menu: mer plats för mappar och filer i Utgångslägen === */

function ui96PanelSaves(){
...
function ui96FindSavesList(){
  return document.getElementById("saves-list") ||
         document.getElementById("saved-list") ||
         document.querySelector("#panel-saves .list") ||
         null;
}

function ui96IsLoadedInfoText(txt){
  txt=String(txt||"").trim().toLowerCase();
  return /(\d+\s+)?(utgångslägen|uppställningar)\s+laddade/.test(txt) ||
         /laddade\s*(✅)?$/.test(txt);
}

function ui96HideLoadedInfo(panel){
  if(!panel)return;
  try{
    Array.prototype.slice.call(panel.querySelectorAll("span,div,p,small")).forEach(function(el){
      var txt=String(el.textContent||"").trim();
      if(txt && ui96IsLoadedInfoText(txt)){
        el.classList.add("tt96-hide-loaded-info");
        el.style.display="none";
      }
    });
  }catch(e){}
...
function ui96FindSearch(panel){
  if(!panel)return null;
  return panel.querySelector("#save-search,#saves-search,#formation-search,#saved-search,input[type='search']") ||
         Array.prototype.slice.call(panel.querySelectorAll("input[type='text']")).find(function(inp){
           var ph=String(inp.placeholder||"").toLowerCase();
           var id=String(inp.id||"").toLowerCase();
           return ph.indexOf("sök")>=0 || id.indexOf("search")>=0 || id.indexOf("sok")>=0;
         }) || null;
}

function ui96FindScopeControls(panel){
  if(!panel)return [];
  var controls=[];
  Array.prototype.slice.call(panel.querySelectorAll("button")).forEach(function(b){
    var txt=String(b.textContent||"").trim().toLowerCase();
    var id=String(b.id||"").toLowerCase();
    if(txt==="mina" || txt==="lagets" || id.indexOf("scope")>=0 || id.indexOf("mine")>=0 || id.indexOf("team")>=0){
      controls.push(b);
    }
  });
  return controls;
...
function ui96LooksLikeFolderButton(b){
  if(!b)return false;
  ...
  if(id.indexOf("new")>=0 || id.indexOf("save")>=0 || id.indexOf("delete")>=0)return false;
  // mappar i den här appen är ofta Allmänt/Alla/+ Mapp eller dynamiska mappnamn nära listans topp
  return true;
}

function ui96FindFolderRow(panel,list,search){
  if(!panel || !list)return null;
  var candidates=[];
  var children=Array.prototype.slice.call(panel.children);
  children.forEach(function(el){
    if(el===list || el===search)return;
    var rect;
    try{rect=el.getBoundingClientRect();}catch(e){rect={height:0};}
    var buttons=Array.prototype.slice.call(el.querySelectorAll("button")).filter(ui96LooksLikeFolderButton);
    if(buttons.length>=1 && rect.height<90){
      var txt=String(el.textContent||"");
      if(txt.indexOf("Mina")>=0 && txt.indexOf("Lagets")>=0)return;
      candidates.push({el:el,buttons:buttons,top:rect.top});
    }
  });

  if(candidates.length){
    // välj raden närmast ovanför listan
...
function ui96MoveSearchToTop(panel,search,scopeControls){
  if(!panel || !search || !scopeControls.length)return;

  var top=document.getElementById("tt96-saves-topbar");
  if(!top){
    top=document.createElement("div");
    top.id="tt96-saves-topbar";
    top.className="tt96-saves-topbar";
  }

  // Försök hitta gemensam befintlig rad för Mina/Lagets.
  var first=scopeControls[0];
  var parent=first.parentNode;
  if(parent && parent!==top && parent.parentNode===panel){
    panel.insertBefore(top,parent);
    top.appendChild(parent);
  }else if(!top.parentNode){
    panel.insertBefore(top,panel.firstChild);
  }

  top.classList.add("tt96-saves-topbar");

  // Om scopekontrollerna inte redan följde med via parent, flytta in dem.
  scopeControls.forEach(function(b){
    if(!top.contains(b)){
      top.appendChild(b);
    }
  });

  if(!top.contains(search)){
    top.appendChild(search);
  }

  search.classList.add("tt96-search");
  try{search.placeholder=search.placeholder||"Sök";}catch(e){}
}

function ui96CompactFolderRow(panel,list,search){
  var row=ui96FindFolderRow(panel,list,search);
  if(!row)return;
  row.classList.add("tt96-folder-row");
...
function ui96ApplyCompactSavesMenu(){
  var panel=ui96PanelSaves();
  if(!panel)return;
  var list=ui96FindSavesList();
  if(!list)return;

  panel.classList.add("tt-v96-compact-saves");

  var search=ui96FindSearch(panel);
  var scopes=ui96FindScopeControls(panel);
  if(search && scopes.length)ui96MoveSearchToTop(panel,search,scopes);

  ui96CompactFolderRow(panel,list,search);
  ui96HideLoadedInfo(panel);
  ...
  try{
    Array.prototype.slice.call(list.querySelectorAll(".row")).forEach(function(row){
      row.classList.add("tt96-compact-row");
    });
  }catch(e){}
}

if(typeof renderSavesList==="function" && !renderSavesList._ui96Wrapped){
  var _renderSavesList_v96=renderSavesList;
  renderSavesList=function(){
    var r=_renderSavesList_v96.apply(this,arguments);
    setTimeout(function(){
      ui96ApplyCompactSavesMenu();
      try{if(typeof ff95PatchTeamLabels==="function")ff95PatchTeamLabels();}catch(e){}
      try{if(typeof ff93RebindFormationButtons==="function")ff93RebindFormationButtons();}catch(e){}
    },0);
    setTimeout(ui96ApplyCompactSavesMenu,120);
    setTimeout(ui96ApplyCompactSavesMenu,400);
    return r;
  };
  renderSavesList._ui96Wrapped=true;
}

document.addEventListener("click",function(e){
  try{
    var tab=e.target.closest && e.target.closest(".tab");
    if(tab && tab.getAttribute("data-panel")==="saves"){
      setTimeout(ui96ApplyCompactSavesMenu,80);
      setTimeout(ui96ApplyCompactSavesMenu,300);
    }
  }catch(err){}
},true);

setTimeout(ui96ApplyCompactSavesMenu,700);
setTimeout(ui96ApplyCompactSavesMenu,1600);

/* === slut v96-formation-compact-menu === */
