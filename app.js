...
document.getElementById("bench-exit-btn").addEventListener("click",function(){matchRoster=[];matchAssignments={};matchVariants=[];activeVariantIdx=0;matchGoals={home:0,away:0};updateGoalDisplay();window._editingMatchId=null;window._editingMatchRef=null;halfMode=0;updateViewBox();players.filter(function(p){return p.team==="home";}).forEach(function(p){p.name="";p.number=0;});render();document.getElementById("bench-bar").classList.remove("active");var go2=document.getElementById("goal-overlay");if(go2)go2.style.display="none";showToast("Laguppst\u00e4llning avslutad");});
...
var match={datum:datum,motstand:motstand||"Ok\u00e4nd",startade:startade,avbytare:avbytare,uppstallningar:JSON.parse(JSON.stringify(matchVariants)),mal_hemma:matchGoals.home,mal_borta:matchGoals.away};var editId=window._editingMatchId||null;if(!editId&&window._editingMatchRef&&window._editingMatchRef.dbId)editId=window._editingMatchRef.dbId;if(editId){match.dbId=editId;fetch(SUPA_URL+"/rest/v1/"+SUPA_TABLE+"?id=eq."+editId,{method:"PATCH",headers:Object.assign({},supaHeaders(),{"Prefer":"return=representation"}),body:JSON.stringify({name:datum+" "+match.motstand,data:match,type:"match",folder:"Allm\u00e4nt"})}).then(function(){var found=false;matcher=matcher.map(function(x){if(String(x.dbId)===String(editId)){found=true;return Object.assign({},match,{dbId:editId});}return x;});if(!found)matcher.push(Object.assign({},match,{dbId:editId}));window._editingMatchId=editId;window._editingMatchRef=matcher.find(function(x){return String(x.dbId)===String(editId);})||null;renderMatchTruppList();renderStatistik();renderMatchHistory();if(typeof renderSparadeMatcherList==="function")renderSparadeMatcherList();showToast("Match uppdaterad!");});}else{fetch(SUPA_URL+"/rest/v1/"+SUPA_TABLE,{method:"POST",headers:Object.assign({},supaHeaders(),{"Prefer":"return=representation"}),body:JSON.stringify({name:datum+" "+match.motstand,data:match,type:"match",folder:"Allm\u00e4nt"})}).then(function(r){return r.json();}).then(function(data){if(data&&data[0])match.dbId=data[0].id;matcher=matcher.filter(function(x){return !(match.dbId&&String(x.dbId)===String(match.dbId));});matcher.push(match);window._editingMatchId=match.dbId||null;window._editingMatchRef=match;renderMatchTruppList();renderStatistik();renderMatchHistory();if(typeof renderSparadeMatcherList==="function")renderSparadeMatcherList();showToast("Match sparad: "+datum+" vs "+match.motstand);}).catch(function(err){cloudStatus("\u274c Fel: "+err.message,"#e84a4a");matcher.push(match);window._editingMatchRef=match;renderMatchTruppList();renderStatistik();showToast("Match sparad lokalt");});}});
var statSelections={};
