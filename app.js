...
document.getElementById("btn-edit-taktik-save").addEventListener("click",function(){if(editingTaktikIdx===null)return;var tk=taktikFilmer[editingTaktikIdx];if(!tk.steps||tk.steps.length<1){showToast("Kan inte spara tom taktikfilm",false);return;}if(tk.dbId){fetch(SUPA_URL+"/rest/v1/"+SUPA_TABLE+"?id=eq."+tk.dbId,{method:"PATCH",headers:Object.assign({},supaHeaders(),{"Prefer":"return=representation"}),body:JSON.stringify({name:tk.name,data:tk,folder:tk.folder||"Allm\u00e4nt"})}).then(function(){cloudStatus("\u2705 Film sparad","#4ae87a");showToast("Film sparad!");});}else{cloudSaveTaktik(tk);}});
...
document.getElementById("new-taktik-ok").addEventListener("click",function(){
  ...
  // Skapa filmen lokalt först. Den sparas i molnet när användaren trycker Spara.
  // Detta undviker en extra 0-stegsfilm i listan.
...
function cloudSaveTaktik(tk){if(!tk||!tk.steps||tk.steps.length<1){showToast("Kan inte spara tom taktikfilm",false);return;}if(tk.dbId){fetch(SUPA_URL+"/rest/v1/"+SUPA_TABLE+"?id=eq."+tk.dbId,{method:"PATCH",headers:Object.assign({},supaHeaders(),{"Prefer":"return=representation"}),body:JSON.stringify({name:tk.name,data:tk,folder:tk.folder||"Taktik"})}).then(function(){cloudStatus("\u2705 Film sparad: "+tk.name,"#4ae87a");showToast("Film sparad!");});return;}if(!tk.folder)tk.folder="Taktik";var body=JSON.stringify({name:tk.name,data:tk,type:"taktikfilm",folder:tk.folder});fetch(SUPA_URL+"/rest/v1/"+SUPA_TABLE,{method:"POST",headers:Object.assign({},supaHeaders(),{"Prefer":"return=representation"}),body:body}).then(function(r){return r.json();}).then(function(data){if(data&&data[0]&&data[0].id){tk.dbId=data[0].id;cloudStatus("\u2705 Film sparad: "+tk.name,"#4ae87a");showToast("Film sparad!");renderTaktikList();}else cloudStatus("\u274c Fel","#e84a4a");}).catch(function(){cloudStatus("\u274c Anslutningsfel","#e84a4a");});}
