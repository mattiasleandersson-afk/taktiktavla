...
/* === v121-formation-owner-share-fix: Utgångsläge/Mina får samma ägarfallback som Taktik ===
   Bas: v120.
   Problem: äldre utgångslägen kan ligga i Mina men ha gammalt ownerId.
   Då ska delning/behörighet ändå tillåtas om ägarnamn + lag matchar.
*/

function tt121Profile(){
  try{
    if(typeof getUserProfile==="function"){
      var p=getUserProfile();
      if(p)return p;
...
function tt121Norm(v){
  return String(v||"").trim().toLowerCase();
}

function tt121Meta(obj){
  try{if(typeof fileMetaV10==="function")return fileMetaV10(obj)||{};}catch(e){}
  if(!obj)return {};
  return obj._meta || obj.meta || (obj.state&&obj.state._meta) || (obj.data&&obj.data._meta) || {};
}

function tt121SameTeam(p,m){
  var pt=tt121Norm((p&&p.teamId)||(p&&p.teamCode)||(p&&p.teamName));
  var mt=tt121Norm((m&&m.teamId)||(m&&m.teamCode)||(m&&m.teamName));
  return !!(pt&&mt&&pt===mt);
}

function tt121IsMine(obj){
  var p=tt121Profile();
  var m=tt121Meta(obj);

  // Äldre filer utan metadata ska behandlas som mina, annars låses de fel.
  if(!m || (!m.ownerId && !m.ownerName && !m.teamId && !m.teamCode))return true;
  ...
  if(
    p &&
    tt121Norm(p.ownerName) &&
    tt121Norm(m.ownerName) &&
    tt121Norm(p.ownerName)===tt121Norm(m.ownerName) &&
    tt121SameTeam(p,m)
  ){
    return true;
  }

  // Extra fallback: samma namn och filen är inte uttryckligen delad från annan ägare.
  if(
    p &&
    tt121Norm(p.ownerName) &&
    tt121Norm(m.ownerName) &&
    tt121Norm(p.ownerName)===tt121Norm(m.ownerName) &&
    m.sharedWithTeam!==true
  ){
    return true;
  }
...
function tt121IsTeamShared(obj){
  var p=tt121Profile();
  var m=tt121Meta(obj);
  if(!p||!m)return false;
  // För Lagets ska även mina egna delade filer kunna synas.
  return tt121SameTeam(p,m) && !!m.sharedWithTeam;
}

function tt121UpdateShareMeta(data,share,canEdit){
  var d=JSON.parse(JSON.stringify(data||{}));
  var meta=d._meta||{};
  var p=tt121Profile();

  // När jag ändrar behörighet tar jag också över metadata till aktuell profil.
  // Då försvinner gamla ownerId-problemet permanent för den filen.
  if(p){
    meta.ownerId=p.ownerId;
    meta.ownerName=p.ownerName;
    meta.teamId=p.teamId || p.teamCode || p.teamName;
...
function tt121PatchFormationShare(s,share){
  if(!s||!s.id){
  ...
  var newState=tt121UpdateShareMeta(s.state,share,false);
  s.state=newState;
  ...
  fetch(SUPA_URL+"/rest/v1/"+SUPA_TABLE+"?id=eq."+s.id,{
    ...
    showToast(share?"Utgångsläge delat med laget":"Utgångsläge inte längre delat");
    try{saveScope=share?"team":"mine";}catch(e){}
    try{currentFolder="Alla";}catch(e){}
    setTimeout(function(){
      try{cloudLoadSaves();}catch(e){}
      try{if(typeof renderSavesList==="function")renderSavesList();}catch(e){}
...
// Överskriv samma V10-filter även för Utgångsläge.
// Obs: detta påverkar både sparade formationer och taktikfilmer eftersom V10 använder gemensamma helpers.
// Därför behåller vi samma logik som v120: Mina = fallback-ägare, Lagets = delad i samma lag.
try{isMineV10=tt121IsMine;}catch(e){}
try{isSameTeamSharedV10=tt121IsTeamShared;}catch(e){}
try{
  isFileVisibleInScopeV10=function(obj,scope){
    return scope==="team" ? tt121IsTeamShared(obj) : tt121IsMine(obj);
  };
}catch(e){}
try{
  isReadOnlyFileV10=function(obj){
    return !tt121IsMine(obj);
  };
}catch(e){}
try{updateShareMetaV10=tt121UpdateShareMeta;}catch(e){}
try{patchFormationShareV10=tt121PatchFormationShare;}catch(e){}

// Taktikdelningen från v120 ska också fortsätta använda samma ägarbedömning,
// men behåll v120:s egen patchfunktion om den finns.
try{
  if(typeof tt120PatchTaktikShare==="function"){
    patchTaktikShareV10=tt120PatchTaktikShare;
  }
}catch(e){}
