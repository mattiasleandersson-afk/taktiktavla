...
/* === v165-direct-taktik-save-only: ersätt bara SPARA taktikfilm ===
   Bas: v159.
   Rör endast cloudSaveTaktik.
   Rör INTE:
   - cloudLoadTaktik
   - renderTaktikList
   - isMine/isReadOnly/filter
   - dela/kopiera
   - editor/animation/fullscreen
*/

(function(){
  if(window.__tt165DirectTaktikSaveOnly)return;
  window.__tt165DirectTaktikSaveOnly=true;
  ...
  function currentTaktik(){
    try{
      if(typeof editingTaktikIdx==="number" && editingTaktikIdx>=0 && taktikFilmer && taktikFilmer[editingTaktikIdx]){
  ...
  function existingMeta(tk){
    if(!tk)return {};
    return tk._meta || (tk.data&&tk.data._meta) || {};
  }
  function cleanForSave(tk){
    var p=profile();
    var x=clone(tk||{});
    var old=existingMeta(tk)||{};
    var m=clone(old)||{};

    if(!Array.isArray(x.steps))x.steps=[];
    ...
    // Behåll befintligt delningsläge, men default false för nya filer.
    m.sharedWithTeam=!!old.sharedWithTeam;
    m.teamCanEdit=!!old.teamCanEdit;
    m.updatedAt=new Date().toISOString();
    ...
    // Saker som inte ska styra ägarskap efter save.
    delete x._readOnly;
  ...
  function applyBack(target,saved){
    if(!target||!saved)return saved;
    Object.keys(target).forEach(function(k){delete target[k];});
    Object.keys(saved).forEach(function(k){target[k]=saved[k];});
    return target;
  }
  function markClean(tk){
    try{if(typeof markTaktikSavedCleanV67==="function")markTaktikSavedCleanV67(tk);}catch(e){}
    try{taktikDirtyV17=false;}catch(e){}
    try{window.taktikDirtyV17=false;}catch(e){}
  }
  ...
  cloudSaveTaktik=function(tk){
    ...
    target=tk || currentTaktik() || target;

    if(!target.steps || target.steps.length<2){
      showToast("Lägg till minst ett steg innan du sparar filmen",false);
      cloudStatus("⚠️ Minst ett steg krävs för att spara taktikfilm","#e8c84a");
      return;
    }

    var clean=cleanForSave(target);
    var rowBody={
      name:clean.name,
      data:clean,
      type:"taktikfilm",
      folder:clean.folder||"Taktik"
    };

    var id=clean.dbId || clean.id || target.dbId || target.id;
    var method=id?"PATCH":"POST";
    var url=id
      ? SUPA_URL+"/rest/v1/"+SUPA_TABLE+"?id=eq."+id
      : SUPA_URL+"/rest/v1/"+SUPA_TABLE;

    cloudStatus("Sparar taktik...","#7aaa88");
    ...
    .then(function(rows){
      ...
      applyBack(target,clean);

      try{
        var found=(taktikFilmer||[]).find(function(x){
          return (x===target) || (x.dbId&&clean.dbId&&String(x.dbId)===String(clean.dbId));
        });
        if(found && found!==target)applyBack(found,clean);
        if(!found)taktikFilmer.unshift(target);
      }catch(e){}

      try{if(playback&&playback.tk)playback.tk=target;}catch(e){}
      try{taktikScope="mine";}catch(e){}
      try{currentTaktikFolder="Alla";}catch(e){}
      try{renderTaktikList();}catch(e){}
      markClean(target);

      cloudStatus("✅ Film sparad: "+clean.name,"#4ae87a");
      showToast("Film sparad!");

      // Medvetet ingen cloudLoadTaktik här.
      // Testet ska avgöra vad som händer först efter riktig refresh.
      return target;
    })
    .catch(function(err){
      cloudStatus("❌ Kunde inte spara taktik: "+err.message,"#e84a4a");
      showToast("Kunde inte spara taktik",false);
      try{taktikDirtyV17=true;}catch(e){}
      throw err;
    });
  };
...
(function(){
  ...
  function dedupe(list){
    var seen={},out=[];
    (list||[]).forEach(function(tk){
      var key=tk.dbId ? "db:"+tk.dbId : "local:"+String(tk.name||"")+"|"+String(tk.folder||"");
      if(seen[key])return;
      seen[key]=true;
      out.push(tk);
    });
  ...
  isFileVisibleInScopeV10=function(obj,scope){
    ...
    return oldVisible?oldVisible(obj,scope):true;
  };

  var oldReadOnly=typeof isReadOnlyFileV10==="function"?isReadOnlyFileV10:null;
  isReadOnlyFileV10=function(obj){
    if(isTaktik(obj))return readOnly(obj);
    return oldReadOnly?oldReadOnly(obj):false;
  };
...
/* === v171-mine-editor-readonly-override: Mina -> editorn ska alltid vara redigerbar ===
   Bas: v167.
   Problem:
   - Filen finns i Mina efter refresh men editorn sätter den ändå som skrivskyddad.
   Orsak:
   - tt76/startPlayback sätter tk._readOnly baserat på egna readonly-checkar inne i editorn.
   Fix:
   - När en taktikfilm öppnas från Mina markeras just den öppningen som redigerbar.
   - Alla readonly-checkar för den öppnade filmen returnerar false.
   - När samma fil öppnas från Lagets förblir den skrivskyddad.
*/

(function(){
  if(window.__tt171MineEditorReadonlyOverride)return;
  window.__tt171MineEditorReadonlyOverride=true;
  ...
  function scopeIsTeam(){
    try{return typeof taktikScope!=="undefined" && taktikScope==="team";}catch(e){}
    return false;
  }
  function scopeIsMine(){
    try{return typeof taktikScope==="undefined" || taktikScope!=="team";}catch(e){}
    return true;
  }
  function markEditableOpen(tk){
    if(!isTaktik(tk))return;
    var id=idOf(tk);
    if(id)editableOpenIds[id]=true;
    tk._openedFromMineV171=true;
    tk._forceEditableV171=true;
    clearReadonly(tk);
  }
  function markTeamOpen(tk){
    if(!isTaktik(tk))return;
    var id=idOf(tk);
    if(id)delete editableOpenIds[id];
  }
  function isEditableOpen(tk){
    if(!isTaktik(tk))return false;
    if(tk._forceEditableV171 || tk._openedFromMineV171)return true;
    var id=idOf(tk);
    return !!(id && editableOpenIds[id]);
  }
  function clearReadonly(tk){
    ...
    try{
      if(playback && playback.tk){
        var a=idOf(playback.tk), b=idOf(tk);
        if((a && b && a===b) || playback.tk===tk){
          delete playback.tk._readOnly;
          delete playback.tk._openedFromTeam;
          delete playback.tk.readOnly;
          delete playback.tk.readonly;
          delete playback.tk._readonly;
          playback.tk._forceEditableV171=true;
          playback.tk._openedFromMineV171=true;
        }
      }
    }catch(e){}
  ...
  function repairEditor(){
    var tk=currentTk();
    if(!isTaktik(tk))return;
    if(!isEditableOpen(tk))return;

    clearReadonly(tk);
    unlockEditorButtons();

    // Viktigt: vissa gamla UI-uppdateringar sätter klassen igen.
    // Kör dem först om de finns, rensa sedan igen.
  ...
  // Läsbarhet i listan: i Mina ska taktikfilm inte visas med öga/skrivskydd.
  if(typeof isReadOnlyFileV10==="function" && !isReadOnlyFileV10._tt171HardOverride){
    var oldRO=isReadOnlyFileV10;
    isReadOnlyFileV10=function(obj){
      if(isTaktik(obj)){
        if(scopeIsMine())return false;
        if(isEditableOpen(obj))return false;
      }
      return oldRO.apply(this,arguments);
    };
    isReadOnlyFileV10._tt171HardOverride=true;
  }

  // Editorns kärncheck. Detta är den viktigaste.
  if(typeof tt76IsReadOnly==="function" && !tt76IsReadOnly._tt171HardOverride){
    var old76=tt76IsReadOnly;
    tt76IsReadOnly=function(tk){
      tk=tk || currentTk();
      if(isTaktik(tk) && isEditableOpen(tk)){
        clearReadonly(tk);
        return false;
      }
      return old76.apply(this,arguments);
    };
    tt76IsReadOnly._tt171HardOverride=true;
  }

  // Äldre skrivskyddslager.
  if(typeof isReadOnlyTaktikV74==="function" && !isReadOnlyTaktikV74._tt171HardOverride){
    ...
    isReadOnlyTaktikV74._tt171HardOverride=true;
  }

  if(typeof tt132IsReadOnly==="function" && !tt132IsReadOnly._tt171HardOverride){
    ...
    tt132IsReadOnly._tt171HardOverride=true;
  }

  if(typeof tt78IsReadonly==="function" && !tt78IsReadonly._tt171HardOverride){
    ...
    tt78IsReadonly._tt171HardOverride=true;
  }

  // När man öppnar från Mina: markera innan gamla startPlayback/tt76 hinner sätta _readOnly.
  if(typeof startPlayback==="function" && !startPlayback._tt171HardOverride){
    ...
    startPlayback=function(idx){
      ...
      setTimeout(repairEditor,0);
      setTimeout(repairEditor,80);
      setTimeout(repairEditor,250);
      setTimeout(repairEditor,700);
      return r;
    };
    startPlayback._tt171HardOverride=true;
  }

  // Om det finns separat editöppning.
  if(typeof openEditTaktik==="function" && !openEditTaktik._tt171HardOverride){
    var oldOpen=openEditTaktik;
    openEditTaktik=function(idx){
      try{
        var tk=taktikFilmer && taktikFilmer[idx];
        if(isTaktik(tk)){
          if(scopeIsTeam())markTeamOpen(tk);
          else markEditableOpen(tk);
        }
      }catch(e){}
    ...
    openEditTaktik._tt171HardOverride=true;
  }

  // Om listan renderas om i Mina, rensa readonly på synliga egna arbetsobjekt.
  if(typeof renderTaktikList==="function" && !renderTaktikList._tt171HardOverride){
    var oldRender=renderTaktikList;
    renderTaktikList=function(){
      try{
        if(scopeIsMine()){
          (taktikFilmer||[]).forEach(function(tk){
            if(isTaktik(tk))clearReadonly(tk);
          });
        }
      }catch(e){}
    ...
    renderTaktikList._tt171HardOverride=true;
  }

  // Sista säkerhet: medan editorn är öppen, håll öppningen redigerbar.
  setInterval(function(){
    try{repairEditor();}catch(e){}
  },500);

  window.tt171RepairEditor=repairEditor;
})();

/* === slut v171-mine-editor-readonly-override === */


/* === v172-saved-id-load-fix: sparade egna taktik-id:n ska laddas i Mina ===
   Bas: v171.
   v171 fixade att öppning från Mina inte blir skrivskyddad.
   Kvar: ny sparad film kan försvinna efter refresh.
   Fix:
   - claimar dbId/id direkt efter cloudSaveTaktik
   - cloudLoadTaktik inkluderar claimade id:n i Mina
   - rör inte editorns v171-fix
*/

(function(){
  if(window.__tt172SavedIdLoadFix)return;
  window.__tt172SavedIdLoadFix=true;

  var CLAIM_KEY="tt_taktik_owned_ids_v2";
  ...
  function isMine(obj){
    return isClaimed(obj) || sameOwnerId(obj) || sameNameAndTeam(obj);
  }
  function isTeamShared(obj){
    return !!(sameTeam(obj) && meta(obj).sharedWithTeam);
  }
  ...
  // 1. Claim efter save.
  if(typeof cloudSaveTaktik==="function" && !cloudSaveTaktik._tt172ClaimWrapped){
      ...
      try{
        if(ret && typeof ret.then==="function"){
          return ret.then(function(saved){
            ...
            if(!id){
              try{
                if(typeof editingTaktikIdx!=="undefined" && taktikFilmer && taktikFilmer[editingTaktikIdx]){
                  id=idOf(taktikFilmer[editingTaktikIdx]);
                }
              }catch(e){}
            }
            if(!id){
              try{if(playback&&playback.tk)id=idOf(playback.tk);}catch(e){}
            }
            if(id)claimId(id);
            return saved;
          });
    ...
    cloudSaveTaktik._tt172ClaimWrapped=true;
  }

  // 2. Behörighet/filter för taktikfilm med claimstöd.
  var oldIsMine=typeof isMineV10==="function"?isMineV10:null;
  isMineV10=function(obj){
    if(isTaktik(obj))return isMine(obj);
    return oldIsMine?oldIsMine(obj):true;
  ...
  var oldRO=typeof isReadOnlyFileV10==="function"?isReadOnlyFileV10:null;
  isReadOnlyFileV10=function(obj){
    if(isTaktik(obj))return readOnly(obj);
    return oldRO?oldRO(obj):false;
  };

  // 3. Load: inkludera claimade id:n.
  cloudLoadTaktik=function(){
    try{cloudStatus("Laddar taktik...","#7aaa88");}catch(e){}
    return fetch(SUPA_URL+"/rest/v1/"+SUPA_TABLE+"?type=eq.taktikfilm&order=id.desc",{headers:supaHeaders()})
      .then(function(r){
      ...
      .catch(function(err){
        console.warn("v172 cloudLoadTaktik failed",err);
        try{cloudStatus("❌ Kunde inte ladda taktik: "+err.message,"#e84a4a");}catch(e){}
        try{renderTaktikList();}catch(e){}
      });
  };

  window.tt172ClaimCurrentTaktik=function(){
    try{
      var tk=null;
      if(typeof editingTaktikIdx!=="undefined" && taktikFilmer)tk=taktikFilmer[editingTaktikIdx];
      if(!tk && playback&&playback.tk)tk=playback.tk;
      var id=idOf(tk);
      if(id){claimId(id);showToast("Markerad som min fil");}
    }catch(e){}
  };
